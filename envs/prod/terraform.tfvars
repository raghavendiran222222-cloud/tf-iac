subscription_id     = "00000000-0000-0000-0000-000000000000" # Replace with actual subscription ID
resource_group_name = "rg-bdt-mvp-prod-eus2-001"
location            = "eastus2"
environment         = "prod"

vnet_address_space = ["10.20.0.0/16"]

app_sku   = "P1v3"
mysql_sku = "GP_Standard_D2ds_v4"

mysql_admin_username = "mysqladmin"
# mysql_admin_password — provide via TF_VAR_mysql_admin_password env var or HashiCorp Vault

storage_replication  = "GRS"
enable_resource_lock = true

log_analytics_workspace_id = "" # Required for prod: set to Log Analytics workspace resource ID

tags = {
  Application         = "BDT MVP"
  DevOwner            = "team@bdtmsd.com"
  BusinessOwner       = "team@bdtmsd.com"
  Environment         = "prod"
  DataClassification  = "Sensitive"
  BusinessCriticality = "High"
  IACRepository       = "https://github.com/bdtmsd/tf-iac"
  CostCenter          = ""
  Compliance          = ""
}
